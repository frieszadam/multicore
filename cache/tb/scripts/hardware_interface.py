...
class HardwareInterface:
  ...
  def __init__(self, interface_name):
    ...
    self.receive_data_len = self.addr_width_p + 2*self.data_width_p + 2
  ...
  def send(self, we, addr, be=0, wdata=0):                
    ...
    trace += self.get_bin_str(0, self.packet_len-self.send_data_len) + "_"
  ...
  # recv data
  def recv(self, cache_data, mem_v, mem_we=0, mem_addr=0, mem_data=0):
    if (mem_v):
      print(f"RECV MEM  : cache_data=0x{cache_data:08X}, mem_we={mem_we}, mem_addr=0x{mem_addr:08X}, mem_data=0x{mem_data:08x}")
    else:
      print(f"RECV      : cache_data=0x{cache_data:08X}")

    trace = "0010_"
    trace += self.get_bin_str(cache_data, self.data_width_p) + "_"
    trace += self.get_bin_str(mem_v, 1) + "_"
    trace += self.get_bin_str(mem_we, 1) + "_"
    trace += self.get_bin_str(mem_addr, self.addr_width_p) + "_"
    trace += self.get_bin_str(mem_data, self.data_width_p) # REVISIT consider testing entire evicted block or just one word
